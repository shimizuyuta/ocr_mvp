...
import { BusinessCardData } from '@/lib/schema';
...
export async function POST(req: NextRequest) {
  try {
    const { businessCard, spreadsheetId }: SaveToSheetsRequest = await req.json();
    ...
    return NextResponse.json<SaveToSheetsResponse>(
      { 
        success: false, 
        message: error instanceof Error ? error.message : 'スプレッドシートへの保存中にエラーが発生しました' 
      },
      { status: 500 }
    );
  }
